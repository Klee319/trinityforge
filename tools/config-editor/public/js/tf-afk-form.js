  ...
  window.buildAfkSection = function buildAfkSection(afkData) {
    ...
    root.appendChild(card(
      ...
      [
        ...
        grid([
          numField(working, "warn-before-seconds", {
            label: "予告カウントダウンの秒数 (warn-before-seconds)", int: true,
            desc: "AFK判定・自動キックの何秒前から予告カウントダウンを出すか。0で予告しません。"
              + "AFK突入の通知は突入した後にしか出ないため、予告が無いとプレイヤーには前触れなく報酬が止まったように見えます。"
              + "AFK前は「AFK判定まで」、AFK中は「自動キックまで」を数えます(キックしない設定ならAFK中は出しません)。"
              + "カウントダウンは判定間隔とは別に毎秒更新されます。idle-seconds 以上にすると保存時にエラーになります"
              + "(常時カウントダウンが出るため、Java側は idle-seconds-1 へ引き下げます)。"
          }),
          boolField(working, "warn-title", {
            label: "予告時に画面中央のタイトルも出す (warn-title)",
            desc: "true で予告の開始時に画面中央へタイトルを1回出します(毎秒出し直すと点滅して読めないため1回だけ)。"
              + "false ならアクションバーの数字だけになります。"
          })
        ])
      ]
    ));
