...
(function () {
  ...
  const STAT_DESCRIPTIONS = {
    ...
    "armor-defense-rate": "バニラ防具値(点数)へ加算される。combat/damage.yml の vanilla-armor.defense-rate-per-point"
      + "(1点あたり1.5%、上限80%)を通して軽減率になる。割合ではないので %矯正(÷100)の対象外。"
      + "パーク/スキルツリーが配る軽減率は defense-rate のほう。",
    "defense-rate": "防御率。ダメージへ (1 - 防御率 x (1 - 相手の貫通率)) を掛ける。"
      + "3つある乗算軽減(防御率/耐性/被ダメージ軽減)のうち、相手の貫通率で打ち消される唯一のもの。"
      + "バニラ防具値から算出される軽減率と合算される。",
