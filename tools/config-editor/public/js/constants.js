  ...
  const FIELD_GROUPS = [
    ...
    },
    {
      ...
      fields: [
        ...
        { id: "level-cutoff.under-level.item-threshold", label: "高レベルモブ判定のレベル差", kind: "int", desc: "(モブのLv - プレイヤーの戦闘Lv) がこの値以上でTF追加ドロップを入手不可にする。つまり【モブのほうが高レベル】なときに効く側(上の「低レベル狩り判定のレベル差」と逆向き)。-1(既定)でこの足きりは無効。経験値には一切影響しない。" }
      ]
    },
