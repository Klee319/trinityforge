  ...
  const FIELD_GROUPS = [
    ...
    },
    {
      ...
      fields: [
        ...
        { id: "level-cutoff.under-level.exp-decay-per-level", label: "経験値倍率の逓減量(レベル差1毎・高レベルモブ側)", kind: "number", desc: "発動後、閾値を1レベル超えるごとに経験値倍率からこの値を引く(線形逓減)。出荷値0.1なので、閾値20から10レベル差が開いた30差で経験値0になる。0にすると逓減なし。" },
        ...
        { id: "level-cutoff.under-level.bonus-per-level", label: "格上を倒したときの報酬上乗せ(レベル差1毎)", kind: "number", desc: "自分より高レベルのモブを倒したとき、レベル差1につき経験値とTF追加ドロップ確率をこの割合ぶん増やす(0.06なら+6%)。⚠効くのは上の「高レベルモブ判定のレベル差」に届かない差だけで、届いた差は足きりが優先される。0で無効。出荷値0.06。狙いはEMダイナミックダンジョンで高いレベルを選ぶ理由を作ること(選択レベルはボスの強さにしか効いておらず報酬に無関係だった)。" },
        { id: "level-cutoff.under-level.bonus-cap", label: "報酬上乗せの上限", kind: "number", desc: "上の上乗せの頭打ち。0.5なら最大+50%。0にすると上乗せ機能ごと無効。出荷値0.5(=レベル差9以上で頭打ち)。" }
      ]
    },
