...
public final class CraftQualityConfig {
     ...
     *
     * <p>経路が2つとも 0 のときだけ落とす: 片方だけ 0 なら「作業台では効かないが儀式では効く」で、
     * ステ自体は生きているため隠してはいけない。
     */
    public Set<String> inertSpreadStatKeys() {
        ...
        if (workbench.upswingScale() == 0.0 && ritual.upswingScale() == 0.0) {
            inert.add(UPSWING_STAT_KEY);
        }
        if (workbench.downswingReductionScale() == 0.0 && ritual.downswingReductionScale() == 0.0) {
            inert.add(DOWNSWING_STAT_KEY);
        }
        return Set.copyOf(inert);
    }

    /** 上振れ増加ステの canonical キー({@code CraftQualityService} が読むキーと同一)。 */
    public static final String UPSWING_STAT_KEY = "craft_upswing_bonus";

    /** 下振れ抑制ステの canonical キー({@code CraftQualityService} が読むキーと同一)。 */
    public static final String DOWNSWING_STAT_KEY = "craft_downswing_reduction";
