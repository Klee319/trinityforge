...
| **W-214** | **パーティクルシードを金床形式で付与できるようにする**（2026-08-24 受領・未着手） | 作業台では付与できない、の対応 |
| **W-215** | **今どんなパーティクルシードを持っているかを GUI で確認する手段を作る**（2026-08-24 受領・未着手) | W-214 とセットで設計 |
...
1. **jar 配備**（停止後）:
   - TF `TrinityForge/build/libs/TrinityForge-0.1.0-SNAPSHOT-all.jar`（16,633,153 bytes / 08-24 23:51。W-216 良薬・W-210〜213・**W-218 スキルツリーの配置**・W-204〜206・盾 W-163・図鑑 W-157/158 ほか多数の未配備修正を含む）。**⚠️ 08-24 02:26 に配備済みの jar は `394ec97`（一括伐採の追加ドロップ）より 18 分古い** ── リンゴ/金リンゴが一括伐採で出ないのはこれが理由なので、この配備で解消する。`836b9b6`（経験値瓶の反転）も同様に**再ビルドしてから**配る
