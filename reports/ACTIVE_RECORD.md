...
| W-148 | エンダーマンで経験値（TFのスキルEXP）が入らなかった人が場所を移動しても入らなかったらしい。同じ場所狩りの減衰措置のバグ疑い | 🔍 **W-117 の続報。同一地点の減衰は否定（4点）／`level-cutoff` も不発（レベル差2）**。残るのは場所非依存の ×0.25 × 日次逓減 と、0 になり得る2経路。**恒久対策＝係数の可視化を提案（未実施）**（下記） |
