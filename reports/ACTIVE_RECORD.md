...
| **W-32** | **メイスのダメージに落下高さボーナスが乗っているか確認し、乗るなら火力をそれ込みで調整する** | バニラのメイスは落下距離で加算される。TF 側の式が二重加算/未加算のどちらかを先に切り分ける |
| **W-33** | **杖の火力が他武器に比べて低すぎる** | 「控えめ」が意図だが、現状は控えめを超えて弱い。tier ごとの実効 DPS で比較してから調整する |
