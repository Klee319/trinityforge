...
**残っている判断**（データは触っていない）: 台帳には無限ソース核の行が 3 つある。

| 行 | 実体 | 扱い |
|---|---|---|
| `BEACON#500001` | フォークが返すアイテム本体 | **生きている。消すと見た目が消える** |
| `NETHER_STAR#500001` | 設置ブロックの表示頭 | **生きている。同上** |
| `NETHER_STAR#5445` | 旧 `materials.yml` 時代の採番（W-201 で移設した際の残り） | **どこからも出ない死んだ行**。ただし移設前に入手された個体が手元にあると、消した瞬間その個体だけ見た目が失われる |
