...
| W-195 | ソースリンクにもベリーと同様に一括でくべたい | ✅ 実装（**操作が変わる**: 通常＝一括 / スニーク＝1個） |
...
#### W-195 ソースリンクの一括投入（**操作が変わる**ので周知が要る）

> 「ソースリンクにアイテムをくべる時にソースベリーと同様に一括でくべられるようにしてほしい」

一括投入は**元からあったが「スニーク時だけ」**（`player.isSneaking() ? hand.getAmount() : 1`)。
ソースジャーへのソースベリー投入（`SourceJar#convertibleBerries`）は**無条件で一括**なので、
同じ「くべる」操作で意味が逆だった。さらにヴォルカニックの燃料は原木・石炭ブロックのような
**設置できるアイテム**が多く、スニーク＋右クリックは設置操作と競合するので、
一括をスニーク側に置くのは操作としても無理があった。

**変更後**: 通常の右クリック＝**手持ちスタック全部** / スニーク＋右クリック＝**1個**。

1個だけ焼べる手段は残す。上位ソースリンクの燃料は単価が桁違い
（`custom:source_engine` = 3,000万）で、握ったまま右クリックした瞬間にスタック全部が
消える逃げ道が要るため。

対象は燃料を受け取る 3 種（ヴォルカニック / アルケミカル / ミセリアル）。
ボタニカル・バイタリックは受動生成でアイテム投入の口を持たない。
判定は `Sourcelink#feedCount` へ集約した（3 実装が同じ式を各々持つと片方だけ直す事故が起きる）。

ジャー側と違って「あふれる一段階手前で止める」上限は掛けていない。ソースリンクのバッファ上限は
既定が int 上限＝実質無制限で、`addToBuffer` がクランプした分は警告ログに残るため、
ジャーのような「無言で消える」損失にならない。
...
- ArsPaper フォーク `a4c072e`（`origin/feat/trinityforge-fork` へ push 済み）
...
- **配備が要る**: `fork-handoff/arspaper/fork/build/libs/ArsPaper-1.0.0.jar`（1,208,405 バイト / 08-24 00:07）。
