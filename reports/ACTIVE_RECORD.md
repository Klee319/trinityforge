...
| **W-36** | **トライデント系・槍系が三人称視点で 2D アイテムモデルになる** | リソースパック側。トライデントは `in-hand`/`thrown` の描画経路がバニラで特殊なので、モデル定義の欠落を疑う |
| **W-37** | **鍵を使ったときに出る本のタイトルが内部 ID 表記** | ダンジョンの表示名にする。**同型の不具合が他にもある**（`/tf settings` の称号ボタンが ID 表示だったのは 2026-08-05 に修正済み） |
| **W-38** | **ツルハシに射撃ダメージが付くなど、不適正なツールへエンチャントが付与される** | 付与対象の判定を洗う |
| **W-39** | **ネザライト化するとエンチャントが剥がれる**（2026-08-05 追加報告） | スミシングのアップグレードで meta 継承が壊れている疑い。W-19 ① の「`NetheriteUpgradeGuard` が 12 件中 7 件を除外」と関係するか要確認 |
| **W-40** | **クリエイティブインベントリのタブにカスタムアイテムが出ない**（2026-08-05 追加報告） | 武器/道具/素材など適したカテゴリへ登録する |
