...
| W-198 | ソースリンクの生成倍率を 2 の階梯乗にしたい | ✅ 実装（無印 2 〜 最上段 512。**config 配備が要る**。転送レートは据え置き） |
...
|---|---|---|---|---|---|---|---|---|---|
| 生成量（新） | 2 | 4 | 8 | 16 | 32 | 64 | 128 | 256 | 512 |
| 生成量（旧） | 1.0 | 1.5 | 2.0 | 2.5 | 3.0 | 3.5 | 4.0 | 5.0 | 6.0 |
| 転送レート（据え置き） | 1.0 | 2 | 4 | 6 | 8 | 12 | 16 | 24 | 32 |

最上段で約 **85 倍**の強化。転送レートは指示に無いので触っていない。
lore の「生成量 xN」も 40 段すべて追随させ、無印 5 種には表示行を新設した。

⚠ **無印にも倍率が乗るようになった**。それまで `ShippedSourceLadderTest` が
「無印は倍率キーを一切持たない」を不変条件として固定していたので、
**「転送倍率は持たない / 生成倍率は 2.0」へ意図的に緩めてある**。
代わりに「段ごとにちょうど 2 倍」を固定するテストを足した（551 件・失敗 0・スキップ 0）。

⚠ **生成量が転送レートを大きく追い越した**（最上段で生成 x512 / 転送 x32）。
以前は「生成は素材効率そのものなので速度より緩く」という設計だったが、その前提はもう無い。
律速はほぼ完全に転送側になる。バッファ上限は int 上限＝実質無制限なので生成分が消えることは
ないが、**ジャーへ流れ込む速さは変わらない**（生成を増やしても回収は速くならない）。
...
- ArsPaper フォーク `0476faf`（push 済み）。jar は 1,209,497 バイト / 08-24 01:29
