...
### 実サーバ報告バッチ（2026-08-24 受領 第35陣。W-201〜W-208）
...
| W-206 | 一括採掘の連鎖分でも追加ドロップが抽選されていない | ✅ 修正（**TF jar と config 配備が要る**。W-204 と同型。**手置きの鉱石は母数に入れない**） |
| W-207 | メイン鯖で装備（防具）の経験値が入らない | ✅ 真因確定 → **ユーザー判断で仕様として据え置き（修正なし）** |
| W-208 | リソパ管理で無限ソース核（`functional-items` 由来）の割り当てを削除できない | ✅ 修正（**設定エディタのみ**。jar も config 配備も不要。同時に「次に誰かが保存したら台帳ごと消える」潜在事故も塞いだ） |

### 実サーバ報告バッチ（2026-08-24 受領 第36陣。W-210〜W-213）

| 番号 | 報告 | 状態 |
|---|---|---|
| W-210 | ドミニオンワンドで繋がっている経路のパーティクルが見にくい（「おそらく広がりが広がりすぎ」） | ✅ 修正（**ArsPaper jar と配備先 yml の書き換えが要る**。ワンド所持中に描く仕組み自体は動いていて、**密度・粒の大きさ・寿命の3つが同時にズレて線として読めなくなっていた**） |
| W-211 | ソースリンクの転送速度が生成量に見合っていない | ✅ 修正（**ArsPaper jar と配備先 yml の書き換えが要る**。**排出が完全な定額だったので「1点あたり0.1秒」が燃料の価値に関係なく固定**され、階梯を上げてもこの比率は永久に改善しなかった） |
| W-212 | 称号がネームタグと同期せず遅れて追従する／Y 座標をあと 0.3 下げたい | ✅ 対応（**TF jar と config 配備が要る**。Y は config で下げられるようにした（**負値を書けなかったのが真因**）。追従は補間長を本体と同じ 3tick へ。**F5 で自分の称号を見たときの遅れは原理的に消えない**） |
| W-213 | スキルツリーを編集して解放可能レベル・ノード接続が変わるとリセットされる（それは可)が、その分の SP が消失していそう | ✅ 修正（**TF jar が要る**。**実データで消失を確認: 14人が計30点を死蔵**。ノードを消しても perk 行は残るが、GUI・効果・振り直しは全部「今あるノード」起点なので**存在しないのに支払い済みのまま**だった） |
| W-214 | パーティクルシードを作業台で付与できない → 金床形式にしたい | ⏳ 未着手 |
| W-215 | 今どんなパーティクルシードがあるかを GUI で確認する手段が無い | ⏳ 未着手 |

#### W-213 真因: 解放は「perk 行」、支払いは「別勘定」なので、ノードを消すと支払いだけが残る

**「リセットされる」は実装された挙動ではなく、消えたノードの perk が見えなくなっただけ。**
そして支払いは別のテーブルに残るので、その分の SP は二度と戻らない。

| | 置き場所 | ノードを消したとき |
|---|---|---|
| 解放したという事実 | `player_perk_states` の行 | **そのまま残る** |
| 支払った点数 | `player_point_balances.spent_points` | **そのまま残る** |
| GUI 表示・バフ・専用効果 | 全部「今あるノード」から引く | **消える**（＝リセットに見える） |
| 残高 | `available = earned - spent` を毎リロード書き戻す | **消えた分だけ永久に減る** |

とどめは救済経路が両方とも同じ穴を持っていたこと。**振り直し**
（`NativePerkService#resetUnderLock`）も**管理コマンドのレベル編集**
（`NativeProgressionAdminService#applyLevelEdit`）も `tree.nodes().values()` を回すので、
**ノードが無い perk は剥がされず・返却もされない**。つまり
「リセットされた気がするので振り直してみる」をやっても回収できない。

**実データで裏を取った**（`Main_Server/plugins/TrinityForge/player_progression.db` を読み取り専用で参照）。
配備先のスキルツリー 16 本のノード ID と、保持している perk 1,447 行を突き合わせた結果:

- 死蔵 **30 行 / 14 人**（1〜4 点ずつ）。内訳は伐採 `A-2-1`(14) `A-2-2`(9) `A-2-3`(3) `A-2-4`(2) と
  鍛冶(Ars) `A-3`(2)。いずれも**今のツリーに存在しないノード**。
- 一方、残高そのものは 27 人全員で `available + spent == earned` かつ
  `spent == 支払額の合計` と整合していた。**つまり帳簿は正しく、「支払い済みで存在しない解放」が
  正しく計上され続けている**のが消失の実体。壊れた勘定を探しても見つからない。

修正は掃除役 `SkillTreePerkPruner` の追加（起動時と `/trinityforge reload` の直後に走る）。
消えたノードの perk 行を落とし、購入時に記録した支払額（`purchase_cost`）を
`available += / spent -=` で戻す。**レベル・EXP・プレステージ段には一切触らない。**

安全弁を3枚重ねた。ここを緩めると「yml が一時的に壊れた回に全員の解放を剥がす」という、
SP が数点戻らないより桁違いに悪い事故になる:

1. 直前のスキルツリー読み込みが**完全成功**した回だけ走る（`SkillTreeConfig#lastLoadOk` を新設。
   成否の記録は出口 1 箇所に集約したので、分岐が増えても書き忘れが起きない）。
   1 ファイルでも警告が出た回は掃除ごと見送る。
2. **ツリーが読めていないスキルの perk には触らない**（プレフィックスが一致するツリーが
   1 本も無いものは「判定不能」として放置）。ツリー 0 本のときは何もしない。
3. プレステージ perk（`*_perk_ng<段>`）とルート perk はノードを持たないので対象外。

順序も効く。`/trinityforge reload` では**曲線の再計算より先**に掃除する。
先に `spent` を減らしておけば直後の `ProgressionCurveReconciler` が
`available = earned - spent` を書き戻す時点で返却分が残高へ乗る。逆順だと
「`spent > earned` なので available を 0 に切り詰め」の警告経路と噛み合わず、
次のリロードまで残高が食い違ったままになる。

**検証**: 新規テスト 12 件（`SkillTreePerkPrunerTest` 8＝判定の表、
`SkillTreePerkPrunerRefundTest` 4＝インメモリ SQLite で実際に決済）。
`resetTreeAloneNeverRecoveredIt` が**修正前の穴そのもの**を固定している
（振り直し後に `woodcutting_perk_a_2` の行と 1 点が取り残されることを assert し、
そのあと掃除で回収されることまで見る）。
TF 全体は **4,639 件・失敗 32・スキップ 2**。失敗 32 は全部
出荷 yml 由来（ARS_MAGIC のレイアウト衝突、`fishing_bonus` の消失、`skill-exp` の値、
item-stats/catalog 系）で、**他セッションが未コミットで編集中の yml が原因**。
自分が触った `skilltree/runtime` の新規 12 件と `progression` 系は緑。

#### W-212 真因: `nametag-clearance` に負値が書けず、下げようとすると逆に上がっていた

報告は「同期していない」と「あと 0.3 下げたい」の2件だが、**バグは後者だけ**だった。
`SpecialRewardsConfig` は負値を「不正」として既定 0.4 へ戻していたので、
すでに 0 まで下げていた運用ではそこから先へ進めず、**下げようとすると逆に 0.4 上がる**。
`TitleDisplayService` 側も `clearance >= 0` を条件に書いていて二重に潰していた。
下限 -0.35（これ以上下げると名前を覆う。W-174 の再発）まで**そのまま使う**ようにして、
出荷値を -0.3 にした。

追従の遅れは**パケットの間隔ではない**。実サーバの `paper-1.21.11.jar` から
`net/minecraft/world/entity/EntityType.class` を取り出して `javap -p -c` で読んだところ、
`text_display` は `updateInterval(1)` で登録されている（＝毎 tick 送っている）。
ズレていたのは補間長で、**クライアントはプレイヤー本体を約 3tick かけて補間する**のに対し
称号の `teleport_duration` は 1 だったので、称号だけが先に目的地へ着いていた
（W-135 の「更新間隔に合わせる」を**別の間隔**に合わせてしまっていた）。
`title-teleport-duration`（既定 3・0〜10）として config 化。

⚠ **F5 で自分の称号を見たときの遅れは、ここを何にしても消えない。**
自分の本体だけはクライアントが予測して即描くのに対し、称号はサーバ由来で必ず往復ぶん遅れる。
他人から見えている位置はズレていない。
⚠ **プレイヤーへ騎乗させれば構造的にズレは消えるが、ネームタグが消える**ので戻さないこと
（W-153 → W-174 で実際にやって撤去した。`TitleDisplayServiceTest` が機械的に禁止している）。

#### W-211 真因: 排出が完全な定額なので「1点あたり0.1秒」が燃料の価値と無関係に固定されていた

`Sourcelink#drainBuffer` は `min(buffer, max-per-transfer × 階梯倍率 × コア倍率)` を出すだけだった。
既定は 50 ÷ 100tick なので **1 点あたり 2tick = 0.1 秒**。ここで肝心なのは、

- `items.<id>.transfer-multiplier`（排出）と `yield-multiplier`（生成）は**全階梯で同じ値**
  （無印 2 / II 4 / II_b 8 / … / VI 512）

なので、**階梯を上げても「1点あたり0.1秒」は 1 ミリ秒も変わらない**という点。
＝ 上位ソースリンクを作っても高価値燃料は入り切らないままで、
「レートを上げたいなら台数を並べる」しかなかった（K-16 が半分だけ残っていた）。

実測（定額のみ・バッファを空にするまでの時間）:

| 投入量 | 定額のみ | 割合 0.10 | **割合 0.25（採用）** |
|---|---|---|---|
| 100（溶岩バケツ） | 10 秒 | 10 秒 | 10 秒 |
| 2,187（圧縮薪 3x）| 3 分 40 秒 | 2 分 | **1 分 5 秒** |
| 4,500（ソースの欠片）| 7 分 30 秒 | 2 分 35 秒 | **1 分 15 秒** |
| 140,000（圧縮薪 1 スタック）| **3 時間 53 分** | 5 分 20 秒 | **2 分 15 秒** |
| 30,000,000（ソース機関）| **約 35 日** | 9 分 35 秒 | **3 分 50 秒** |

修正は `transfer.sourcelink.drain-ratio`（既定 0.25）を足して
**`max(定額, バッファ × 割合)`** を出す形にしただけ。設計上の要点が3つある:

1. **割合の項に階梯倍率を掛けない。** 掛けると生成量倍率と再び同率で伸びて、
   「階梯を上げても比率が変わらない」元の構造にそのまま戻る。
2. **小口では定額が勝つ**ので、溶岩バケツ 1 個の体感（10 秒）は修正前と同じ。
   割合だけにすると尻尾が 1 点ずつになって永久に終わらない。
3. **ジャーの空きを超えて出しても損失はない。** 注ぎ切れなかった分のうちバッファ由来は
   `SourceYield#refundToBuffer` でバッファへ戻る（受動生成分だけは従来どおり捨てる）。
   つまり「ジャーの容量が実質的な上限」という元設計は壊れていない。

判断は `SourceDrainPolicy`（Bukkit 非依存の純関数）へ切り出した。
**所要時間の表そのものをテストで固定**してあるので、割合排出を外すと
「定額のみ」の値（220 秒 / 3,000,000 秒）に戻って落ちる。

#### W-210 真因: 「見にくい」は密度・粒の大きさ・寿命の3つが同時にズレていた

ワンド所持中に経路を描く仕組み（`SourceNetworkParticleTask`）は正常に動いていた。
2026-08-01 に負荷を毎秒 8,064 → 528 粒子へ落としたときの値が、
**負荷は軽いが線として読めない**組み合わせだった:

1. **間隔 1.0m に対して粒の大きさが 0.8〜1.2**（1.0 はバニラのレッドストーン粒と同寸）。
   → 大きい玉が 1 ブロックおきに並ぶだけで、線ではなく飛び石に見える。
   粒の大きさは Java にハードコードされていて**設定で直せなかった**。
2. **描き直しが 20tick（1 秒）なのに DUST の寿命は約 8〜40tick（クライアント側の乱数）**。
   → 次の描画までに半分近くが消えるので**点滅する**。
   ⚠ ここは「周期を伸ばすほど軽くなる」と「寿命を超えると切れる」がぶつかる箇所で、
   **10tick を超えさせてはいけない**。
3. **view-distance 48m × 最大 16 経路＋隣接供給の全描画**。
   → ソース設備の周りでは一帯が点だらけになり「今どれを繋いだのか」が埋もれる
   （報告の「広がりが広がりすぎ」はここ）。

修正は「**細く・詰めて・切れない線を、近くの数本だけ**」へ振り直し:

| | 初版 | 2026-08-01 | **2026-08-24** |
|---|---|---|---|
| 描き直し | 10tick | 20tick | **10tick** |
| 間隔 | 0.5m | 1.0m | **0.5m** |
| 粒の大きさ | 0.8〜1.2（固定） | 同じ | **0.45（`dot-size` で設定可）** |
| 描画範囲 | 48m | 48m | **24m** |
| 経路数 | 64 | 16 | **8** |
| 粒子/秒 | 約 8,064 | 約 528 | **約 1,008** |

密度を 2 倍にした代わりに範囲と本数を半分にしたので、合計は初版の 1/8 に収まる。
終端マーカーと隣接供給の粒も `dot-size` からの相対値（×1.8 / ×1.15）にしたので、
**実機で見ながら 1 箇所だけ動かせば全部が追随する**（今後は jar 再ビルド不要）。
終端マーカーの散らし幅も 0.15 → 0.05 へ（散らすと線が滲んで太く見える）。

**⚠ 既定値を動かしただけでは実サーバに届かない。** ArsPaper は
`saveResource(..., false)` なので、jar を差し替えても `/ars reload` しても
配備先の `sourcelinks.yml` は永久に書き換わらない
（[[arspaper-config-saveresource-false-never-updates]] の常連）。
そのための経路が新規 `ops\scripts\apply-sourcelink-transfer-tuning.ps1`:

- 3 バックエンドの `sourcelinks.yml` を直接書き換える（何度流しても同じ結果）
- **旧既定と一致する行だけ**書き換え、誰かが手で調整した値は触らず警告する
- 書き込み前に `.bak-<日時>` へ退避、UTF-8（BOM なし）+ LF で書き戻す
- 検証: HEAD 版の yml を tmp へ複製して実走 → PyYAML で解析し
  `drain-ratio 0.25` / `interval-ticks 10` / `spacing 0.5` / `dot-size 0.45` /
  `view-distance 24` / `max-paths 8` を確認。BOM なし・CR なし・
  `items` 45 件と `volcanic.materials` 65 件が無傷、差分はコメント以外 6 行だけ

**検証**: ArsPaper `95be8b0`。テスト **567 件・失敗 1**。
失敗 1 は `ThreadRitualRecipeConfigTest`（3 段の `max-slots` が単調増加でない）で、
**他セッションが `items.yml` を未コミットで編集している**（`max-slots: 2→1` / `5→3`）ため。
`com.arspaper.source.*` は 104 件すべて緑。jar は `build\libs\ArsPaper-1.0.0.jar`
（1,214,804 バイト・14:21）で、`SourceDrainPolicy.class` と
新しい `sourcelinks.yml` が入っていることを zip の中身で確認済み。

⚠ **フォークのワークツリーには他セッションの未コミット変更が 20 ファイル以上ある**
（`SourceNetwork.java` +85 行、`SourceStorage.java` 新規、`spell/` 配下 7 本など）。
巻き込まないよう自分の 7 ファイルだけを列挙して commit したが、
`SourceNetworkParticleTask` が `SourcelinkTickTask#trackedSourcelinks`（W-105 の
読み取り専用スナップショット・16 行）に依存しているので**それ 1 本だけ同梱**した
（無いと HEAD がコンパイルできない）。

#### W-208 真因: フォークの Java がハードコードした割り当ては台帳から見えない

無限ソース核は material（`BEACON` / 表示頭の `NETHER_STAR`）と CMD（`500001`）を
**フォークの `InfinitySourceCore.java` が固定**していて、CMD スキャン対象の yml
（catalog / item-stats / materials / spellbooks / external-items / sourcejars / sourcelinks）には
**1 行も存在しない**。`functional-items.yml` の側には `display-name` と `recipe` しか無く、
material も CMD も書かれていない。この結果、次の 2 つが同時に壊れていた。

1. **登録解除が必ず失敗する。** `/api/respack/unregister` は「(material, CMD) を参照する config を
   全部書き換える」実装なので、実体が config に無いと 1 件も当たらず
   `404「〜を参照するconfigが見つかりませんでした」`＝ 画面には「登録解除に失敗しました」。
2. **（未発覚の潜在事故）次に誰かが editor で config を 1 つ保存した瞬間に、台帳から黙って消える。**
   `reconcileWithUsageDetailed` は台帳を**スキャン結果だけから作り直す**ので、
   スキャンに出てこない行は毎回そのまま捨てられる。台帳から消えると
   `regenerateItemDefinitions` が `beacon.json` / `nether_star.json` の配線も外すため、
   **アセットは 1 枚も消えていないのに見た目だけが元に戻る**（W-201 の再発と見分けがつかない）。

対処は 2 段:

- `cmd-registry.js` に `isExternalSource()` を追加し、**スキャン対象外の由来を持つ行は
  reconcile で落とさない**（同じ (material, CMD) が usage に現れた場合だけ usage 側を採る）。
  引き継ぎ（`movable` / `claimed`）の対象からも外し、assetName の二重使用を防ぐ。
- `/api/respack/unregister` に、config で 1 件も当たらなかったときの経路を追加。
  **台帳の行だけを落として同期する**＝「リソパの配線だけを外す」意味の登録解除になる
  （アイテム自体はフォーク側に残るので、in-game では見た目がバニラに戻るだけ）。

テスト: `tools/config-editor/test/cmd-external-allocation-2026-08-24.test.js`（5 件）。
CMD・respack 系 8 ファイルで 112/112 通過。

台帳にあった無限ソース核の行 3 つのうち、**死んでいた `NETHER_STAR#5445` のみユーザー判断で削除**した
（旧 `materials.yml` 時代の採番。W-201 で `functional-items.yml` へ移設した際の残りで、現在は
どこからも払い出されない）。残る `BEACON#500001`（アイテム本体）と `NETHER_STAR#500001`
（設置ブロックの表示頭）は**フォークが実際に返す生きた行なので消してはいけない**。

削除は台帳 `resourcepack/cmd-registry.json`（729 → 728 行）と
`assets/minecraft/items/nether_star.json` の閾値 5445 の 2 箇所。手で消した状態が
**登録解除ボタンの生成結果と一致すること**を、実パックの複製に `regenerateItemDefinitions`
を走らせて確認済み（item 定義の全ファイルで差分 0）。**反映にはパックの再ビルド／再公開が要る**。
※ 移設前に入手された個体（CMD 5445）が手元にあると、その個体だけ見た目がネザースターに戻る。
...
#### W-207 真因: レベル差の足きりは「新しく始めた職業」を永久にゼロで締め出す（仕様として据え置き）

**「全く入らない」ではない。** メイン鯖の実 DB（`player_progression.db`、読み取り専用）で確認したところ、
`HEAVY_ARMOR` は当日も加算されており直近 7 日で 18 人が更新されている。
止まっているのは **`LIGHT_ARMOR` が Lv0〜5 のまま動かない 6 人**で、
全員 `HEAVY_ARMOR` は Lv59〜73 で毎日伸びている（＝軽装備側だけが死んでいる）。
その 6 人の軽装備側の最終更新は **08-17 / 08-18 / 08-22** で止まっている。

**機構**: 2026-08-22 に入れたレベル差の足きり（E1）。配備値は
`under-level.exp-threshold: 15` / `exp-decay-per-level: 0.067` / **`rate-floor: 0`**
＝ **モブのレベルがそのスキル自身のレベルより 30 以上高いと完全にゼロ**（`MobLevelCutoff#expMultiplier`）。
一方モブのレベルは `combat/mob-types.yml` の配備値で
**オーバーワールド = 拠点からの距離 × 0.02（1500 ブロックで Lv30）/ ネザー = 25 + 距離 × 0.015 /
エンド = 80 スタート**。

つまり **新しく始めた職業（Lv0）は、その人が普段いる場所ではレベル差 30 以上が確定するので
最初の 1pt すら永久に入らない**。防具で目立つのは、装備の系統を替えると Lv0 からやり直しなのに
周囲のモブレベルは高いままだから。⚠️ **同型の締め出しは防具に限らない** ——
弓術（平均 11.2 / 最小 0）など、後から始めるすべての戦闘職に同じことが起きる。

- **ユーザー判断: 仕様として直さない**（2026-08-24）。育てたい職業は拠点近く（低レベル帯）で育てる運用。
- 直す気になったときの最小手は **`combat/damage.yml` の `under-level.rate-floor` を 0 から上げる**
  （config だけで全戦闘職に効き、jar は要らない）。閾値そのものを緩めると
  「低レベルのままハメ殺しで格上を狩る」抑制まで一律で弱くなるので、そちらは避けるほうがよい。

#### W-206: 一括採掘（`VeinMiningListener`）も同型 → 同じ方式で修正

`VeinMiningListener` も連鎖分は `BlockBreakEvent` を発火しないので、drop-tables は
**起点 1 ブロック分しか引いていなかった**。伐採と違い起点そのものが鉱石＝抽選対象なので
「1 回も出ない」ではなく「連鎖しても回数が増えない」＝**1 個ずつ手で掘るより損をする**。

- **修正**: `vein-mining.chain-drop-rolls-max`（新設・既定 8）を上限として、
  「**連鎖で壊した自然生成ブロックの数**」と上限の小さいほうの回数だけ追加で引く。
  落とす位置は叩いた起点（鉱脈の奥に取り残さない）。
- ⚠ **手置きの鉱石は母数に入れない。** 起点の抽選（`onBlockBreakDropTables`）が設置ブロックを
  除外しているのと同じ規約。ここを外すと **W-199 で開いた「回収 → 並べて設置 → 一括破壊」の
  経路で追加ドロップだけを無限に引ける**。数えるのは**壊す前**（破壊後は設置マークが残らない）。
- **回帰テスト**: `VeinMiningListenerTest` に 4 件（上限 3 で 3 回 / 壊した数 2 で 2 回 /
  上限 0 で 0 回 / 手置きは 0 回）、`MiningGimmickConfigTest` に 2 件。
  抽選回数は**ワールドのアイテム数ではなく `itemResolver` の呼び出し回数**で数えている
  —— 鉱石そのもののドロップと混ざるため。
  **`VeinMiningListenerTest` 17 件 / `MiningGimmickConfigTest` 12 件・いずれも失敗 0・skip 0**。
- **検証**: TF フルテスト **4621 件・失敗 32・skip 2**（失敗数は W-204 着手前と同じ 32）。

---


### ユーザー要望（2026-08-24 受領。W-216）

> 「特殊アイテム（スキルノードの楔枠）でEXP解呪の良薬を3種類追加。=>レシピは仮置きでOK
> ・10%(-40%)まで減らせるもの ・25%(-25%)まで減らせるもの ・50%(初期値)まで減らせるもの」

| 番号 | 要望 | 状態 |
|---|---|---|
| W-216 | 特殊アイテムに「EXP解呪の良薬」を3種追加 | ✅ 実装（**TF jar が要る**。**数字は「減る側」の上限**なので引き戻す先の取得倍率は 90% / 75% / 50%。**並(50%)は倍率が動かないのに無意味ではない**理由を下に書いた） |

#### 何を「解呪」するのか

`stats/skill-exp.yml` の **`daily-diminishing`（日次逓減）**。直近 24 時間にそのスキルで稼いだ量に応じて
スキル EXP の取得量が段階的に薄まり、**下限は `floor: 0.5`（＝どれだけ稼いでも最悪 50% は入る）**。
プレイヤー側からは「EXP がロックされた」と見える機構で、W-154 で「発動から 24 時間で強制解除」が付いている。

**数字の向きに注意。** ユーザー指定の「10% / 25% / 50%」は**減る側（ペナルティ）の上限**。
出荷値 `floor: 0.5` が「ペナルティ 50% が初期値」なので、対応は次のとおり。

| アイテム | ID | ペナルティ上限 | 引き戻す先の**取得倍率** |
|---|---|---|---|
| EXP解呪の良薬・並 | `exp_cleanse_tonic_lesser` | 50%（初期値） | **50%** |
| EXP解呪の良薬・上 | `exp_cleanse_tonic_greater` | 25% | **75%** |
| EXP解呪の良薬・極 | `exp_cleanse_tonic_supreme` | 10% | **90%** |

ここを逆に読むと「良薬を飲むほど稼げなくなる」ので、実装側は
`ExpCleanseTonic#targetMultiplier()`（＝引き戻す先の倍率）という名前で向きを固定し、
`ExpCleanseTonicReliefTest` が「どの良薬も下限 0.5 を下回らない」を縛っている。

#### 効き方（ユーザー選択 2026-08-24）

- **飲んだ瞬間だけ解除する使い切り。** 恒久的に下限を書き換えるものではないので、飲んだ後また稼げば普通に下がる。
- **全スキル一括。** 逓減はスキルごとに別々に溜まるが、良薬 1 個で全部に効く。
- **目減りしていないときは消費しない。** 誤って飲んで黙って 1 個消えるのが一番きつい壊れ方なので、
  切り下げが 1 件も起きなければ券を返してその旨を出す。

#### 実装で踏んだ罠 2 つ

**① 下限クランプに飲まれて「並(50%)」が無言で効果ゼロになる。**
倍率は `floor` でクランプされるので、「倍率が 0.5 以上になる蓄積量」を素直に解くと**上限なし＝何もしない**になる。
蓄積は下限へ張り付いた後も増え続ける（実サーバでは数百万まで伸びる）ので、
**クランプ前の生の曲線 `decay^n` で段数を解き、張り付き始める手前まで削る**のが正しい。
こうすると倍率は 0.5 → 0.531 と 1 段だけ上がり、**自然回復までの時間が実際に縮む**。
これが並の良薬の唯一の価値で、テストもそこを見ている（`DailyExpDiminishing#maxAmountFor`）。

**② メモリだけ削ると再ログイン／サーバ移動で効果が消える。**
`DailyExpWindowStore#save` は「メモリと保存済みの**大きい方**」を残す（Velocity のサーバ移動で
移動先の join が移動元の quit より先に起きるため、後退させない設計）。
つまりメモリを削って保存しても**保存済みの大きい値がそのまま勝つ**。
専用の切り下げ入口 `capAmounts` / `capStored` を足し、**メモリ → DB の順**で削る
（逆順だと間に挟まった定期保存が削る前の値を書き戻す）。DB 側は SQLite なので非同期タスクへ逃がす。

#### 出したもの

- `items/catalog.yml`: 3 件を新設。**CMD は台帳（`resourcepack/cmd-registry.json`）へ払い出し済み（261〜263）。**
  リソースパック側のモデルはまだ無いので**見た目は素材そのまま**（HONEY_BOTTLE / POTION / DRAGON_BREATH）。
- **レシピは仮置き**（ユーザー了承済み）。儀式レシピで、素材は
  **`ars-smithing.exp-per-material` に既に行がある品だけ**で組んである
  —— 表に無い素材を混ぜると儀式 EXP が無言で取りこぼされる（`ShippedRitualMaterialExpCoverageTest` が検出）。
- 設定エディタの**特殊アイテム画面**へ 3 件を追加（見た目とレシピだけ編集可。**倍率は editor 側に無い**）。
- 回帰テスト `ExpCleanseTonicReliefTest` **9 件・失敗 0・skip 0**。
  TF フルテスト **4656 件・失敗 32・skip 2**（失敗 32 は着手前と同数＝**他セッションの未コミット yml 由来**で、
  `exp_cleanse` を含む失敗はゼロ）。

---


### ユーザー要望（2026-08-24 受領。W-217）魔法追加の仕様草案

> Discord（りる ↔ Klee）の会話履歴に基づく。**これは草案であって実装ではない。** 承認前。

**前提として確認できたこと（実コードで裏取り済み）**

- ArsPaper のグリフは **`glyphs.yml` の 120 件**（形態 10・効果 80・増強ほか）。
  1 件 = **Java クラス（`com/arspaper/spell/{form,effect,augment}/`）＋ `glyphs.yml` のエントリ**の対。
  **config だけで新しい魔法は生えない。** 逆に、既存グリフの数値・階梯・解放コストは config で全部動く。
- 形態は `ProjectileForm` / `TouchForm` / `SelfForm` / `UnderfootForm` / `OverheadForm` /
  `OrbitForm` / `BurstForm` / `BeamForm` / `LingerForm` / `WallForm` の 10 本。
- BAN は `ban.yml` の **4 件だけ**（`solar` 日輪 / `lunar` 月輪 / `sonic_boom` / `journey` 旅路）。
- **`flight`（飛行のスレッド）は既に存在する**（`threads.yml:130`、`flight: true`。
  `thread-sets.yml` にもセット効果の枠がある）。新規作成ではなく**制限を足す**話。
- **エリトラは現状「重装備」扱い。** `UseSkillDefaults.isArmorPiece` はエリトラを防具に数えるが、
  `isLightArmorName` は `LEATHER_` / `CHAINMAIL_` / `GOLDEN_` の 3 接頭辞しか軽装にしない。
- **エリトラに防御力を乗せるのは可能**（`EquipmentSlotResolver:126` がエリトラを胸当てスロットとして解決するので、
  `item-stats.yml` のエントリがそのまま効く）。Klee の「エリトラに防御力は可能」は正しい。
- **マナ消費軽減は既にステータス画面に出ている**（`stats/lore.yml` の
  `mana-cost-reduction-flat`「マナ消費効率(実)」/ `-percent`「マナ消費効率(率)」。カテゴリ `ars`、
  **`hide-when-zero: true`** なので 0 の人には出ない）。
...
| B | 剣を飛ばす魔法 | 新規効果グリフ | 中 | **形にはできるが「害悪の上位互換」になりやすい**。差別化案を下に |
...
| H | 再生魔法の解禁 | 要調査 → 新規効果グリフ | 小〜中 | **「banしてる」対象が特定できていない**（下記） |
| I | エリトラに軽装備レベルを乗せる | TF（分類＋item-stats） | 中 | **前向きに検討**（ユーザー優先度）。分類の一本化が要る |
| J | エリトラのマナ回復を儀式で 5 段階 | Ars 儀式＋TF item-stats | 中 | **エリトラだけ 5 段階**でよい（各装備分は不要） |
| K | 飛行のスレッドに使用可能レベル | TF（ゲート機構の新設） | 中 | **既存の単一スキル制限では書けない**（下記） |
...
会話で出た Klee の懸念 3 つはどれも実装前に潰せる。

- **「統合版の描画が難しい」** → 飛ぶ本体を `ItemDisplay`（鉄／金／ダイヤの剣）にする。
  統合版は Display エンティティを描けるので、投射体そのものを見せる必要は無い。
- **「旋回の扱い」** → `orbit`（旋回）と組ませると多重ヒットで壊れる。**旋回とは非互換にする**か、
  旋回時は 1 周 1 ヒットに落とす。`max-augments` で `orbit` を書かなければ機構的に組めない。
- **「単純にダメージだと害悪で良くね？になる」** → ここが本質。**素の威力で差を付けない**。
  差別化案（どれか 1 つ選ぶ）:
...
**単体特化・対ボス特攻**。`beam`（照射）との差は「貫通する線」ではなく「1 体だけに全部入る」こと。
...
`glide`（滑空）の隣。**りるの狙いは「指向（`speed_boost`）と合わせて移動する」**なので、
**落下速度を落とさない**（＝速いまま落ちて無傷）のが要点。緩落の劣化版にしないこと。

#### F. 屠畜魔法

会話で出た具体値: **生肉 15・革 15 / 羊毛 6 / 羽 8**（確定ドロップ）。
動機は「軽装備のレベル上げで革が欲しい」「羊毛はですさんが村人で使いそう」。

- **要注意**: これは**経済に直接効く**。革 15 確定は牛 1 頭あたりバニラの約 7 倍。
- **草案**: 数値はそのまま入れたうえで、**マナコストを高く**し、
  **同一個体には 1 回だけ**（`fell`（伐採）や `harvest`（収穫）と同じく「効率化」であって「増殖」ではない形にする）。
- **家畜討伐の農業 EXP** が同時に入る点に注意（2026-08-20 に 80% へ調整済みの経路）。
  屠畜で EXP まで美味しくすると放置狩りの新経路になるので、**EXP は乗せない**のが草案の立場。

#### G. 壁を一瞬消す魔法

「通り抜け」は却下済み（Klee「技術的に厳しそう」→ りる了承）。代わりに
**「一瞬だけ壁を消す」**（りる「岩盤も消せるなら」）。

- 既存の `phantom_block`（仮想ブロック）の**逆**。指定範囲のブロックを N 秒間だけ空気に見せ、
  時間が来たら戻す。**クライアント側のブロック変更（`sendBlockChange`）で済ませれば実際には壊さない**ので、
  地形破壊にならず、戻し忘れも起きにくい。
...
会話では「あるにはある／ban してる」→「強いから」→ りる「回復魔法の方が強い（回復は連発できる、
再生は地道）」→ **Klee「じゃあ入れとく」**（＝解禁で合意）。

⚠ **ところが実物が見つからない。**
`glyphs.yml` の 120 件に**再生に相当するグリフは無い**（あるのは `heal`「回復」）。
`ban.yml` の 4 件にも入っていない。つまり「ban してる」が何を指すのかが未特定。
考えられるのは（a）本家 Ars Nouveau にはあるがフォークへ移植していない、
（b）`unlock-gate.yml` / `usage-gate.yml` 側で塞いでいる、（c）`heal` の別形態を指していた、の 3 つ。

**草案**: (a) 前提で **新規効果グリフ `regeneration`**（継続回復）を作る。
...
要望は **耐久値の減りを少なく／速度／防御力**。優先度はユーザー申告で「前向きに検討」。
...
会話の確定事項: **儀式のコアにエリトラを置く**／**5 段階**／**各装備分は不要、エリトラだけでよい**。

- Ars の儀式は `catalog.yml` の `recipe.method: ritual` で書ける（今回の良薬と同じ形）。
...
会話の確定事項: **combat ではなく軽装備・重装備の両方 90**（りるは当初 100 と発言、
Klee「装備にするか」で装備スキルへ）。**全身装備でセット効果**。
