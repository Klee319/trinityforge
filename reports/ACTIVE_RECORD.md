...
| ID | 内容 | 状態 |
|---|---|---|
| W-162 | モブから「深淵のクロスボウ」「深淵の弓」（＝準備中で未出荷のはずの item）がドロップする | 🔨 着手中 |
| W-163 | 盾に効果が無い（構えても構えなくても受けるダメージが変わらない）。**要望**: 盾だけ特殊仕様にして、しゃがみ／構え中だけ設定したステータスが乗るようにしたい | 🔨 着手中 |
| W-164 | スキルの %低下（EXP 逓減）が 24 時間を超えても解除されない。W-154 で「最初に減衰を始めてから 24 時間でリセット」に変えたはずでは | 🔨 着手中 |
| W-165 | 設定エディタの「ダンジョンの挑戦レベルに応じた報酬の増減 (2026-08-18)」の入力欄が**軒並み空欄**なのは仕様か | 🔨 着手中 |
