...
| 対象 | 状態 |
|---|---|
| `dev` HEAD | **`0e0888e`**（`origin/dev` と一致）。2026-08-10 の当セッション分は `42459bc`（J-12 圧縮素材EXP=0）→ `66d621b`（J-14 孤児テクスチャ削除）→ `1b1f2da`（厳選ロール幅・杖の攻撃速度）→ `0e0888e`（M-2 手懐けモブのレベル）。**その手前は別セッションの `8d085f9`/`ac5dd3e`** |
| テスト（2026-08-10 実測） | TF **3836 / 25 failed / 2 skipped**。**25 件は全て他セッションの未コミット yml 由来**（`combat/stat-caps.yml` が作業ツリーで `stat-caps: {}` に潰れている件で 4 件、skilltree 5 件、collection 2 件、gates 2 件、achievements 2 件、lore.md ドリフト、brew、レシピ/儀式ゲート 3 件ほか）。**skip 2 が正常値**は不変 |
