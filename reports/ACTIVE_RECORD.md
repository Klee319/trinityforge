...
| W-148 | エンダーマンで経験値が入らなかった人が場所を移動しても入らなかったらしい。同じ場所狩りの減衰措置のバグ疑い | 🔍 **真因確定・数値判断待ち**。同一地点の減衰ではなく `level-cutoff.under-level`（下記） |
| W-149 | 圧縮ご飯がまだ食べられない（W-131 の再報告） | ✅ **真因確定・修正済（config 再配備待ち）**。コードのバグではなく**配備オーバーレイの巻き戻り**（下記） |
| W-150 | 精錬速度が早すぎる。高速精錬を最大まで解放して170%上昇＝2.7倍のはずが、原木1スタックが3秒程度で終わる | ✅ 対応（下記。ユーザー判断で**「速度+X%」解釈へ統一**） |

#### W-148 エンダーマンで経験値0 — 同一地点の減衰ではなく **レベル差の足きり**

**同一地点の減衰（`spot-diminishing`）は原因ではない。** 3点で否定できる。

1. 判定は「その地点の周囲 `radius` 内・直近 `window-seconds` 秒の撃破数」で、**場所スコープ**。
   移動すれば履歴が範囲外になり倍率は 1.0 に戻る（`LocationExpDiminishing#multiplierAt`）。
2. **0 にならない**。`floor`（配備値 0.4）で下げ止まり、しかも
   `applyToVanillaExp` は `Math.max(1, …)` で**最低1オーブは必ず残す**。
3. 記録するのは撃破のみ（被弾/命中では増えない）。配備値も radius 16 / window 300 /
   threshold 80 / decay 0.02 / floor 0.4 で、報告のような全損は起こせない。

**真因は `combat/damage.yml` の `level-cutoff.under-level`**（レベル差の足きり）。
`KillRewardAdjuster#expMultiplier` → `MobLevelCutoff#expMultiplier` が
**バニラ経験値オーブと TF 戦闘スキルEXPの両方**に掛かる。配備値は

```
under-level: exp-threshold: 15 / exp-rate: 1 / exp-decay-per-level: 0.067 / rate-floor: 0
```

で、`倍率 = clamp(1 − 0.067 × (レベル差 − 15), 0, 1)`。レベル差は
`モブレベル − プレイヤーの戦闘レベル`。

| レベル差 | 15 | 20 | 25 | 29 | **30以上** |
|---|---|---|---|---|---|
| EXP倍率 | 1.00 | 0.67 | 0.33 | 0.06 | **0.00** |

ここへ2つの配備値が噛み合う。

- **ジ・エンドは `base-level: 80`**（`mob-types.yml` の `dimensions.THE_END`）。
  エンダーマンは `level: 0` なのでディメンションの帯そのままで **Lv80**。
- **戦闘レベルは素のスキルレベルよりかなり低い**。`progression/combat-level.yml` の
  pillar は top1÷1.5 / top2÷2.0 / top3÷2.5 / top4÷3.0 なので、
  1スキルLv100で67、2スキルLv50ずつで50、1スキルLv50だけなら33。

つまり **戦闘レベル 50 以下のプレイヤーはジ・エンドのエンダーマンから経験値を1点も得られない**
（満額が要るのは戦闘レベル65＝2スキルLv65ずつ相当）。**足きりはレベル差だけで判定するので、
場所を移しても何も変わらない** — 報告の「移動しても入らなかった」がそのまま説明できる。
同じことが**オーバーワールドのLv80固定ボス**（ウィザー / エルダーガーディアン / ウォーデン）でも起きる。

仕様どおりの動作なので、**数値をどう変えるかはユーザー判断**（この足きりは 2026-08-18 に
「低レベルがハメ殺し／デスルーラーで高レベルモブを倒す」抑制のために対称化したもので、
ジ・エンドが通常進行のコンテンツであることと正面衝突している）。
