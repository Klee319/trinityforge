...
| W-167 | `/tf recipes` にカスタム追加したポーション（幸運など）の醸造レシピを載せる。ソートカテゴリに「醸造」を追加する | 🔨 着手中 |
...
- **足りない情報（これが無いと真因を確定できない）**:
  1. 報告者は **Java 版 / 統合版どちら**か（統合版なら Geyser の破壊時間計算とサーバ判定のズレ＝
     いわゆるゴーストブロックが第一候補。→ `docs/agent-context/bedrock-geyser.md`）
  2. 手に持っていたツールの**正確な内容**（材質・効率レベル・ドリリングのレベル・他のエンチャント）
  3. 「掘れない」の見え方（**ヒビが最後まで行かない** / **一瞬消えて戻る** / **音だけ鳴る**）
  4. しゃがむと掘れるか（ドリリングはしゃがみ中無効なので、**しゃがめば掘れる**なら
     原因はドリリング側に確定する）
