...
| 日付 | 内容 |
|---|---|
| 2026-08-27 | **ホワイトリストを導入した**（`ops/scripts/apply-whitelist.ps1` 追加、`ops/SECURITY.md` §2 更新）。**きっかけは実測**で、Velocity のログに**海外のデータセンター IP（Scaleway 等。実アドレスはリポジトリに載せない）から接続してきて、ワールドへ入る前に切れているアカウントが 2 件**あった＝既にスキャンされている。**置き場所はバックエンド側**（Velocity には標準のホワイトリストが無く jar の追加調達が要る。判断が出たら Velocity 側へ移す）。**名簿はリポジトリに置かない**（public なのでプレイヤー名と UUID を版管理へ入れない）。スクリプトは実行のたびに稼働サーバ自身の `usercache.json` / `playerdata` / ログの `joined the game` から組み立て、控えは `tmp/whitelist/`（gitignore）にだけ残す。**`UUID of player ... is ...` 行だけのアカウントは採用しない**（認証が通っただけでワールドへ入っていない＝スキャナーがここに落ちる）。**統合版（Floodgate）は `whitelist add` で足せない**ので、UUID 直書きのファイル経路が唯一。対象は main / resource の 2 台で、**dev は既に `white-list=true`（Klee319 のみ）なので触らない**。tmp の偽バックエンドで書き込み経路を実走検証済み（名簿・`server.properties`・退避・BOM 無し）。**実サーバへの適用はユーザー実行待ち**（`D:\game` はエージェントの権限ゲートで拒否）。⚠ 検証中に判明: **Windows PowerShell 5.1 の `ConvertFrom-Json` は配列を 1 個のオブジェクトとして出力する**ので、`@(...)` で囲っても件数は常に 1 になり「書き込み後に件数を数える」検算が全部素通りする（詳細は `docs/agent-context/ops-build-deploy.md`） |
