...
| W-199 | 手置きの鉱石でも一括破壊を発動させたい（ただしバニラEXPと職業EXPは入れない） | ✅ 実装（**TF jar が要る**。⚠ 設置済み鉱石は起点のバニラEXPオーブを常に失う／**GTH-02 の増殖経路が開く**） |
...
**「EXP は入らない」側は 4 経路のうち 3 つが既に成立していた**（＝実際に足りなかったのは 1 つだけ）:

| 経路 | 状態 |
|---|---|
| 起点の採取EXP・破壊時バニラEXP（TF） | `NativeSkillExperienceListener` の設置マークガードで既に 0 |
| 連鎖分の採取EXP | 同じガードを `ChainBreakExpGrant` 経由で 1 ブロックずつ通る（手置きの段だけ 0。巻き込まれた自然生成の段は従来どおり入る） |
| 連鎖分のバニラEXPオーブ | `ChainBreakSupport#breakOnce` は `setType(AIR)` なので**そもそも出ない** |
| **起点のバニラEXPオーブ** | **唯一残っていた** → `event.setExpToDrop(0)` を追加 |

`isPlaced` を **HIGH** で読むのが必須。設置マークを消すのは MONITOR の
`NativeSkillExperienceListener` なので、MONITOR で読むと登録順によっては消去後になる
（`GatheringExtraDropListener` / `MiningFortuneListener` が同じ理由で HIGH に居る）。

⚠ **一括破壊の解放や発動可否と関係なく、設置済みの鉱石は起点のバニラEXPオーブを失う。**
`setExpToDrop(0)` はパーク／トグル／ツール／階梯の門より**手前**に置いた。
要望の「すでに一回採掘済み」はパークの有無に依らない性質だから。
...
- TF 本体 `717c105`。push 済み。**TF jar の再ビルドと配備が要る**（config ではない）。
