# TrinityForge — 記録アーカイブ（**残タスクの一次情報ではない**）

現役の残タスク・既知バグ・現在の状態は **`reports/ACTIVE_RECORD.md`** だけを見る。
このファイルは 2026-08-04 の整理で ACTIVE_RECORD から切り出した**過去分**で、用途は 2 つだけ。

1. **同じ調査を二度やらないため**の解決根拠（「なぜそう直したか」「なぜ却下したか」）。
2. **失敗の型**を辿るため（配備漏れ・記録の腐り・テストが緑なのに壊れている、の再発を照合する）。

**書かれている「現在」「未配備」「未コミット」は、すべて書かれた時点の話。** そのまま信じてはいけない。
必ず実コードと `git log` で裏を取る（この運用は 2026-07-27 と 2026-08-01 の棚卸しで
**記録の腐りが計 20 件**見つかったことから来ている）。

追記のルール: **このファイルには追記しない。** 作業記録は ACTIVE_RECORD の §7 に書き、
そこが長くなったら古い順にこのファイルの「作業履歴」先頭へ移す。

---

## A. 解決済み・破棄した残タスク（旧 §2 判断待ち / §3 手を動かせば終わる / §4 既知の問題）

解決根拠つきで残す。**再起票する前にここを検索すること**（「実は既に実装済みだった」が何度も起きている）。
...
**並びは厳密な逆時系列ではない。** 「新しいものを上に」の規約に従った追記と、末尾に足した追記が
混在している（切り出し時点の状態をそのまま保っている）。日付で検索すること。

### 2026-08-24 — ACTIVE_RECORD 全面棚卸しでの移設分（ユーザー指示による一括アーカイブ）

ユーザー指示「既に完了した内容や棄却された内容を順に査読し、完了済みは削除かアーカイブ化して、
判断が必要な残タスクを洗い出す」に基づく移設。**中身は原文のまま**（取り消し線・状態表記も当時のもの）。
現役として残した項目は `reports/ACTIVE_RECORD.md` の §2（判断待ち/情報待ち）・§3（実装タスク）・
§3.5（配備・運用待ち）・§4（既知の問題）を見ること。ここにある「未着手」「残」「配備待ち」の記述は
**移設時点までに本体側へ転記済み**なので、この節から再起票しないこと。

#### 旧 §1 現在の状態（2026-08-10〜08-14 のスナップショット）
...
#### 旧 §3 手を動かせば終わるもの（決着済みの行を含む全文。現役の行は本体 §3 へ転記済み）
...
#### バッチ節の全文（2026-08-04〜08-24 受領。W-28〜W-215・S-1〜S-5 ほか約 60 節）
...
| 番号 | 内容 | 状態 |
|---|---|---|
| W-217 | 魔法追加（マナ削減率の可視化／剣を飛ばす／狙撃(ホーミング)形態／レールガン／着地／屠畜／壁を一瞬消す／再生の解禁／エリトラの軽装備化／エリトラ儀式5段階／飛行スレッドの使用レベル の11件） | ⏳ **草案のみ**（未着手）。**中身は [docs/design/2026-08-24-magic-additions-draft.md](../docs/design/2026-08-24-magic-additions-draft.md) が正** |

着手前にユーザー判断が要る点が5つある（剣飛ばしの差別化／壁消しで岩盤を消せるか／
「banしてる再生魔法」の実体／屠畜に職業EXPを乗せるか／エリトラ儀式にバニラのエリトラを許すか）。
決まったものからこの表を分割して W 項目を切る。

⚠ 調査で判明した中で一番効くのは **「再生魔法が glyphs.yml の120件にも ban.yml の4件にも存在しない」**
（＝「banしてる」の対象が未特定）と、**「エリトラは現状“重装備”扱い」**、
**「飛行のスキル要件は既存の単一スキル制限では書けない」** の3つ。根拠は上のドキュメント §0。
...
### 2026-08-04 — config-editor のレスポンシブ化（375px まで）

commit: `fdb364e`。`dev` へ push 済み。**editor は jar 配備を伴わない**（`public/` の静的資産のみ）。

触ったファイルは `tools/config-editor/public/` の 5 本だけ（`style.css` / `index.html` /
`js/app.js` / `js/respack-view.js` / `js/simulator.js`）。**並行セッションが触っていた
`TrinityForge/src` と `resourcepack/` には一切触っていない。**

**何が壊れていたか（機構レベル）**: このエディタの CSS は「サイドバー 300px 固定 + 十分に広い
`.main`」前提で、87KB に `@media` が 3 つしか無かった。狭い幅での壊れ方は 3 段構え:

1. `.layout { grid-template-columns: 300px 1fr }` が 300px を主張し続ける → 375px 幅で本文は 75px。
2. **`.main { overflow-x: hidden }` なので、あふれは「スクロールで見える」ではなく `クリップ`**。
   入力欄・削除ボタン・保存ボタンが**画面外で永久に押せない**。
3. `.form-label{width:190px}` + セレクト `flex: 0 0 260px`（shrink 0）のような固定幅の横並びが 2 を直接起こす。

**実測（A/B 対照を取った）**: 375px・「武器」タブでカード 6 枚展開時、
**変更前 = 本文幅 75px / クリップされて到達できない要素 928 個**（保存ボタン含む）→
**変更後 = 本文幅 375px / 0 個**。変更前の CSS を一時的に戻して同じ計測をし、直っていることを確認した
（`fix-must-be-proven-not-plausible` の「戻すと落ちる」確認）。

| 幅 | サイドバー | 本文幅 | クリップ |
|---|---|---|---|
| 1440px | 300px 固定（**変更前と一致**） | 1130px（**一致**） | 0 |
| 1200px | 248px | — | 0 |
| 1024px | 216px | 798px | 0 |
| 768px | ドロワー（閉） | 758px | 0 |
| 375px | ドロワー（閉） | 375px | 0 |

11 画面（武器/素材/火力シミュレータ/リソースパック管理/ロア表示/基礎ステ/スキル/スキルEXP/
アチーブメント/モブ定義/モブオーバーライド）で計測。

**やったこと**:

- 900px 以下でサイドバーをオフキャンバス・ドロワー化（ハンバーガー / スクリム / Escape /
  項目選択で閉じる。カテゴリ折りたたみでは閉じない）。
  **吸着位置に px を決め打ちしない**——`.layout`（＝トップバーの下の残り全部）を
  `position: relative` の基準にして `absolute` で貼る。トップバー実高が日本語フォントで変わる問題
  （2026-07-25 の `.layout` 修正と同じ罠）を再発させないため。
  閉状態は `visibility: hidden` にして a11y ツリーからも外す（`read_page` で nav 項目が
  出ないことを確認済み）。
- `auto-fill` グリッドの下限を `minmax(min(Npx, 100%), 1fr)` へ（5 箇所）。狭い幅で自動的に 1 列へ落ちる。
- 700px 以下で固定幅の横並びを縦積みへ（`.form-field` / `.stat-row` / `.drop-item-control` / `.path-row`）。
- **列数の多い表は `.table-scroll` ラッパで包む**。最初 `@media (max-width:700px)` で
  `table{display:block;overflow-x:auto}` にしたが、**768px で CMD 台帳（8 列）が 149px あふれて
  クリップされるのを実測**して方式変更。あふれるかは画面幅ではなく**中身の文字数**で決まるので
  閾値で切り替えてはいけない。ラッパなら表の `width:100%` が温存され広い画面の見た目も変わらない。
- `body` の `height` に `100dvh` を追加（モバイルのアドレスバー分が画面外に出る）。

**テスト**: `npm test` = **1052 tests / 1041 pass / 11 fail**。
**fail 11 件の集合は着手前と 1 行も違わない**（`diff` で完全一致を確認）。
...
③`resize_window` しても **`resize` / `matchMedia` の `change` イベントが 1 回も飛ばない**
（0 件を実測）。②③を知らないとドロワーが「開いたまま」「閉じない」と誤診する。
...
## 2026-08-03 ダンジョンごとの物理/魔法コンセプト（#62）＋ 被弾ダメージの引き上げ（#63）

### #62 ダンジョン単位の `stats:` を新設し、28ダンジョンにコンセプトを割り当てた

**着手前に見つかった構造的な穴**: `combat/mob-overrides.yml` は `level-cutoff:` だけが
「scope 直下（ダンジョン全体の既定）」と「`mobs.<モブid>` 直下」の2階層を持ち、
**`stats:` は `mobs.<モブid>` 直下にしか書けなかった**。ダンジョンの敵は 396 体いるので、
「このダンジョンの敵は魔法型」というコンセプトを 1 体ずつ 396 回書くしか手段が無かった。

`MobOverridesConfig` に **scope 直下の `stats:`** を追加した（`level-cutoff` の2階層構造に合わせた）。

- 解決は 3層 → **5層** に。`base < default の scope直下 < default の mob単位 < world の scope直下 < world の mob単位`
- `level-cutoff` と違い**ブロック採用ではなく項目単位マージ**。scope 直下で `magic-ratio` だけ書き、
  特定のモブだけ `mobs.<id>.stats.max-health` を上書きする、という重ね方ができる
- **`mobs:` に1件も書いていないモブにも効く**のがこの層の存在意義（`applyScopeDefaults`）
- `worldScopeKey` の候補を `scopes` だけでなく **scope直下設定を持つ scope との和集合**に変更。
  これが無いと「`mobs:` を書かずコンセプトだけ指定した scope」が無言で一致しなくなる
  （既存の `level-cutoff` にも同じ潜在バグがあった。ついでに直っている）
- `mobs:` 欠落の警告は「scope直下の設定も無いとき」だけに限定（正しい書き方を誤警告しないため）

**割り当て（29 scope 中 28）**。3種のコンセプトを、敵の耐性と攻撃の型の両方で作る:

| コンセプト | 敵 phys 耐性 | 敵 magic 耐性 | 敵 magic-ratio | 割り当て |
|---|---|---|---|---|
| 物魔両方 | 0.12 | 0.12 | 0.25 | 9（試練1/4/7/10、古橋、登攀路、ネザー荒野、宮殿、北極） |
| 物理推奨（敵=魔法型） | 0.05 | 0.32 | 0.45 | 9（花火、ハロウィン、世界を繋ぐ者、試練3/6/9、灼熱の洞窟、ネザーの鐘、闇の大聖堂） |
| 魔法推奨（敵=物理型） | 0.32 | 0.05 | 0.10 | 10（試練2/5/8、ドワーフ都市、深層鉱山、鉱山、採石場、騎士団の城、下水道、蒸気機関） |

「攻めに使う属性」と「受けに要る属性」が**逆になる**のは意図した設計。同じ属性に全部寄せれば
済む作りにすると結局1本の最適解に戻るため。エンチャント試練1〜10は
「両方 → 魔法推奨 → 物理推奨」を繰り返す梯子にして、一本の装備で登り切れないようにした。

`em_adventurers_guild`（拠点）だけ**意図的に未割り当て** — 中身は DPS 計測用ダミーで、
scope 直下に耐性を書くと測定値が狂う。

### ⚠ magic-ratio を 0.60 で切ろうとして、検算で 0.45 に下げた（非自明・重要）

最初 物理推奨=0.60 / 物魔両方=0.30 / 魔法推奨=0.08 で書いたが、実際のダメージ式に当てて落とした。

**被ダメージは「守備力(flat-defense)を引いてから率を掛ける」ので、物理/魔法に分割された各成分は
それぞれ別の守備力で減算される。ところが `stats/item-stats.yml` を全数確認したところ、
ネザライト系防具は全変種・全部位が `phys-resistance` / `phys-flat-defense` **のみ**で、
`magic-flat-defense` は 1 つも無い**（魔法防御を持つのは守護/魔織シリーズだけ）。

Lv60・ネザライト一式（phys-flat-defense ≈ 49、最大HP ≈ 50）でダンジョンモブの攻撃力を 66 とすると:

| magic-ratio | 物理成分（守備力49を引いた後） | 魔法成分（無対策なら素通り） | 合計 | 耐えられる発数 |
|---|---|---|---|---|
| 0.10 | 59.4 − 48.8 = 10.6 → 6.1 | 6.6 | 12.7 | 約3.9 |
| 0.25 | 49.5 − 48.8 ≈ 0（下限へ） | 16.5 | 約17 | 約2.9 |
| 0.45 | 0（下限へ） | 29.7 | 約30 | 約1.7 |
| 0.60 | 0（下限へ） | 39.6 | 約40 | **約1.3 = 実質2発以下** |

つまり **Lv60 では物理成分がプレイヤーの守備力にほぼ丸ごと吸われ、magic-ratio が事実上
「素通りダメージの割合」そのもの**になっている。0.60 は「対策すれば無傷・しなければ即死」の
両極端になるため上限を **0.45** に下げた。`MobOverridesConfigTest` に
`magic-ratio <= 0.45` のガードを入れ、根拠を yml ヘッダにも書いた。

### #63 被弾を痛くする: `attack-power` に Lv45 以降の第2区間

`combat/mob-import.yml` の `attack.attack-power` に `high-level-from: 45 / high-level-per-level: 0.25`。

- Lv45: 26.5（据え置き。閾値ちょうどでは加算0で連続）
- Lv60: 41.2 → **45.0**（+9%）
- Lv80: 74.5 → **83.2**（+12%）

**+9% で体感が大きく変わる理由**: 上表のとおり守備力を引いた"残り"に率が掛かるので、
攻撃力の増分は残りに対して何倍にも効く。Lv60・魔法推奨ダンジョンの物理成分の残りは
`10.6 → 16.0`（約1.5倍）。**ここは小さく回すレバー**で、大きく回すと即詰みになる。
足りなければ `high-level-per-level` を 0.05 刻みで上げる。
`fixed-damage` を上げるのは禁じ手（守備力で減らせない＝対策不能になる）。

**45+難易度修正で `attack-power` を意図的に触らなかった**（HPと flat-defense だけ上げた）のは
「被ダメージ側を悪化させて詰みを作らないため」だったので、これはその判断の**上書き**にあたる。
今回はユーザーの明示要望による。`ShippedMobImportBreakpointTest` に
「Lv45未満は無干渉」「Lv60で加算が乗っている」「Lv60が素のカーブの1.2倍未満（回し過ぎ検出）」の
3点ガードを追加した。

なお同ファイルの flat-defense のコメントが「レベル1につき+15」と書いてあったが
実値は `high-level-per-level: 150.0`（Lv60 で +2250）で **10倍ずれていた**ので訂正した。

### この波のテスト結果（#62/#63）

- TF 本体 `./gradlew cleanTest test`: **BUILD SUCCESSFUL**、失敗0 / **SKIPPED 2**。
  スキップ2件は既知（`OfflineMobImportRunner`（実 custombosses ツリーが要る）と
  `NativeProgressionStabilizationContractsTest#prestigeRefundUsesLiveYamlCost`）。
- editor `npm test`: **1021件 / 1012 pass / 9 fail**。9件は着手前と**名前まで完全一致**のベースライン
  （player wiki generator ×2、木～ネザライトのツール斧／斧／槍、金ツール耐久、
  `buildSkillExpForm`、ソースジェム防具の魔法防御、ロスレス `mining-gimmick.yml`）。増えていない。

新規テスト（すべて緑）:
- `MobOverridesConfigTest` に scope 直下 `stats:` の 8 件
  （mob エントリ非在でも効く／書いた項目しか触らない／mob単位が項目単位で勝つ／
  world scope が default scope に勝つ／設計図名で全インスタンスに当たる／
  `mobs:` 無しでも skipped にしない／範囲外 magic-ratio は skipped で他項目は生きる／
  **出荷ymlの28ダンジョン全部にコンセプトが乗っていて 0.10/0.25/0.45 の3種が実際に使われている**）
- `ShippedMobImportBreakpointTest` に `attack-power` の 45 区間ガード 1 件
  （Lv45未満は無干渉・閾値で連続・Lv60で加算あり・**Lv60が素のカーブの1.2倍未満**）

### ⚠ 上の #62 は**半分が no-op で、9ダンジョンは方向が逆だった**（同日中に自己申告・修正）

「敵の攻撃タイプは物理/魔法で棲み分けできているか」を確かめる過程で、**自分が同日に入れた #62 の
実装に2つの欠陥**を見つけた。上の表のうち「敵 phys 耐性 / 敵 magic 耐性」の列は**取り消し**。

**欠陥1: scope 直下に書いた耐性は1体にも効いていなかった（丸ごと no-op）**

`MobStatOverride.mergeDefense`（`mobs/MobStatOverride.java:60-67`）は
`override.resistance() != null ? override.resistance() : base.resistance()` という**項目単位マージ**。
scope 直下 → mob 単位の順に適用されるので、**モブ個別が `resistance:` を1つでも持てば scope 側は必ず負ける**。
出荷ymlを数えると `^          (physical|magical):` が 794 行、うち `resistance:` を含むものが 397 行 ——
つまり**ダンジョンモブの実質全数**が自前の耐性を持っており、scope に書いた 0.05/0.32 はどこにも届かない。
`attack.magic-ratio` の方は per-mob 側がほぼ書いていない（ダンジョン内は 0 件）ので**こちらは効いていた**。

**欠陥2: コンセプトは既に存在していて、9ダンジョンで真逆のラベルを重ねていた**

各ダンジョンの先頭に **`# ▼コンセプト: 魔法ビルドが活きる(物理装甲が厚い)`** 等のコメントが
**29個すべてに既にあり**、per-mob の耐性もそのとおりに組まれていた。着手前にこれを読んでおらず、
ダンジョン名から推測してラベルを振ってしまった。実データとの照合結果:

| ダンジョン | 既存 ▼コンセプト（＝実際の耐性） | 初版で振ったラベル | 修正後 |
|---|---|---|---|
| 花火工房 | 魔法ビルドが活きる | 物理推奨 0.45 | **魔法推奨 0.10** |
| エンチャント試練1 | 物理ビルドが活きる | 物魔両方 0.25 | **物理推奨 0.45** |
| エンチャント試練4 | 魔法ビルドが活きる | 物魔両方 0.25 | **魔法推奨 0.10** |
| エンチャント試練5 | 物理ビルドが活きる | 魔法推奨 0.10 | **物理推奨 0.45** |
| エンチャント試練6 | 魔法ビルドが活きる | 物理推奨 0.45 | **魔法推奨 0.10** |
| エンチャント試練7 | 物理ビルドが活きる | 物魔両方 0.25 | **物理推奨 0.45** |
| エンチャント試練10 | 魔法ビルドが活きる | 物魔両方 0.25 | **魔法推奨 0.10** |
| 北極ミニダンジョン | 物理ビルドが活きる | 物魔両方 0.25 | **物理推奨 0.45** |
| 下水道迷宮 | ハイブリッド | 魔法推奨 0.10 | **物魔両方 0.25** |

真逆（花火・試練5・試練6）だと「**物理が通る敵が物理で殴ってくる**」＝攻めも守りも同じ属性で
足りる状態になり、属性を分けた意味そのものが消える。

**修正内容**

- 28 scope 全部から `physical:` / `magical:`（＝死んでいた耐性）を削除。scope 直下に残すのは
  `attack: { magic-ratio: }` **だけ**。守りは per-mob 側だけで表現する（既存の実装がそうなっている）
- 上表の 9 ダンジョンを ▼コンセプトに合わせて振り直し。**28件すべて対応が一致**
  （魔法推奨12 / 物理推奨11 / 物魔両方5、＋拠点1は未割り当てのまま）
- エンチャント試練は結果として **奇数=物理推奨 / 偶数=魔法推奨の交互梯子**になった
  （既存の「課題ごとに要求ビルドが入れ替わる」というコメントと一致する）
- yml ヘッダを書き直し、**「scope 直下に耐性を書くな・書いても per-mob に必ず負ける」**を明記
...
~~**このファイルはコミットしなかった。**~~ → **ユーザー判断で「今すぐそのままコミット」を選択したため `47ceb22` で確定した。**
コミット前に、同梱する2本のテストが HEAD の他ファイルに対しても緑になることを確認している:

- `progression/collection.yml` の `categories.items.mob_parts` の**エントリ一覧は HEAD と作業ツリーで完全に同一**
  （差分はコメント削除・`不死者`→`アンデッド` の改名・`title: "..."`→`title: ...` の引用符正規化だけ）。
  → `ShippedRitualMaterialObtainabilityTest` の判定対象集合は変わらない。
- `items/catalog.yml` の儀式 `pedestal-items` は `      - custom:...` → `        - custom:...` の**再インデントのみ**。
  YAML のブロックシーケンスは親と同じ字下げでも妥当なので**両方とも同じ木に解決する**。
  `warden_tendril` を要求する3本の行は差分にすら現れない（＝未変更）。

なお `collection.yml` にも**同じ編集器保存の署名**が出ている（コメント削除＋引用符正規化）。
`item-stats.yml` だけの問題ではない。

### この節でコミットしたもの（`1a2b810..ebf360f`、`origin/dev` と一致）

`git add -A` は使わず、パスを1本ずつ列挙している。

| commit | 内容 |
|---|---|
| `bec3d6f` | 物理PvP抑制が `BASE` だけを縮めていた件（`scaleModifiersExceptBase` へ集約） |
| `1726daf` | フィールドモブの攻撃力に高レベル区間 Lv45+ を新設（#63）。`dimensions:` は空のまま＋検算コメント |
| `764f787` | `warden_tendril` を踏破ボス3種へ（儀式3本が永久に作成不能だった件） |
| `315c81b` | この節の記録 |
| `47ceb22` | 重装/軽装の防御序列を反転（#17） |
| `ebf360f` | 「作れるのに素材が手に入らない」を掃きで検出するテスト |

### 決着した判断（2026-08-03 ユーザー回答）

1. **`dimensions:` は空のまま確定。** エンドのモブHPが約12〜26倍・EXP帯とドロップ品質も同時に跳ねる
   副作用を避ける。将来刻むなら `THE_END` は 45 ではなく **20〜30**（高レベル区間に入らない範囲）から。
   `ShippedMobTypesLevelBandTest` が空であることを固定済みなので、**決定なしに値が入ることはない**。
2. **`item-stats.yml` は別セッターの編集を同梱したままコミット。** commit message に混入内容を明記した。

### 残る既知の問題（新規タスクとして扱うこと）

- **編集器で yml を保存すると日本語コメントが消える。** 今回 `item-stats.yml` で65行、
  `collection.yml` でも同様の消失を確認した。1件ずつ復元しても**次に同じ画面を開いて保存すればまた消える**ので、
...
## C. 再構成前の §1〜§6（2026-08-04 以前の全文）

2026-08-04 に ACTIVE_RECORD を「現役分だけ」に絞った際、**開いている項目は新しい §2〜§4 へ移した**が、
その過程で要約した長い散文（特に **K-5 の修正プラン全文**と、当時の状態表）をここへそのまま残す。
**残タスクの一次情報ではない。** 現役の項目は必ず `reports/ACTIVE_RECORD.md` を見ること。
