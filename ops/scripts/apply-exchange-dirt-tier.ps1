...
.DESCRIPTION
    ...
    【なぜスクリプトが要るのか】
    ArsPaper は自分では既存の yml を更新しない（saveResource(..., false) ＋
    updateResourceFiles はバージョン文字列が変わったときしか再展開しない）。
    つまり **jar を差し替えても /ars reload しても配備先の glyphs.yml は変わらない**。
    リポジトリ側の編集を届ける経路は ops\launch\deploy-config-head.cmd だけで、
    そちらは **全バックエンドを止めてからでないと流せない**。

    このスクリプトは「止めずに今すぐ入れたい」ときの経路。配備先の実ファイルを直接書き換え、
    各サーバで /ars reload すれば反映される。ここで入れた編集は次回の config 配備で
    guard-deployed-config.ps1 が drift として拾い、リポジトリへ書き戻す（W-177）ので、
    黙って巻き戻されることはない。
