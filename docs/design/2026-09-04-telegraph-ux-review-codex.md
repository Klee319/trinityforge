...
| 3 | 輪郭だけ・5段バーの可読性は未保証 | **採用**: 固定地点の中心点滅、残り秒の数値併記、粒子は対象プレイヤーへ個別送信。**出荷前の実機受入条件を設計正本へ明記**（Java／統合版 ×低粒子・暗所・水中・一人称・4人同室を各1回以上確認。`▮▯` が潰れる場合は `telegraph-bar-style: ascii` へ切替）。実機検証そのものは未実施のまま |
...
| 6 | 2.6 b/s の机上式が遅延を扱えない | **採用**: `許容半径 ≤ 状態別速度 × max(0, 詠唱秒 − D95) − 安全余白` へ式を置換。**D95 は仮置き 0.35 秒**で、実測して差し替える前提を設計正本へ明記 |
...
| 10 | 予算が傍観者に効かない・`+1` に潰れる | **採用**: **致命予告は脅威圏内の全員ぶんの枠が同時に取れないと撃たない**（`telegraph-lethal-atomic`、既定 true）。非致命予告は主対象必須＋脅威圏内はベスト・エフォート予約のまま |
| 11 | 半径 32 の告知が無関係者へ | **採用**: 受信者＝主対象＋脅威圏内＋anchor 水平 12m |
| 12 | 空振り硬直・選択リズム | **採用**: `whiff-stagger-seconds`（中ボス・ボス限定、8秒に1回、雑魚には付けない）＋直近3回の行動語に重みを付ける技選択のリズムを実装 |
| 13 | 例外・退出時の掃除が未保証 | **採用**: 一度だけ走る終端処理に集約、例外はレート制限ログ |

**2026-09-04 午後 更新**: #3・#6・#10・#12 を採用へ更新。詳細は設計正本
`docs/design/2026-08-31-dungeon-concept-rework.md` と実装仕様
`docs/design/2026-09-02-telegraph-mechanics-spec.md`「機構 7〜10」節に反映済み。
