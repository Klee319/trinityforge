...
## PowerShell 5.1: `ConvertFrom-Json` は配列を「1 個のオブジェクト」として出す（2026-08-27）

書き込みの検算を**全部素通りさせる**ので、JSON を扱う ops スクリプトでは必ず踏む。

```powershell
@(ConvertFrom-Json '[]').Count                       # → 1（0 ではない）
@(ConvertFrom-Json '[{"a":1},{"a":2}]').Count        # → 1（2 ではない）
@((ConvertFrom-Json '[{"a":1},{"a":2}]') | % { $_ }).Count   # → 2（正しい）
```

Windows PowerShell 5.1 は配列を**列挙せずに 1 個の `Object[]` として**パイプへ出す。
`@(...)` で囲っても件数は 1 のまま。「書き込んだあと読み直して件数を数える」形の検査は
**常に不一致になる**か、逆に**常に 1 件だと思い込む**。パイプへ流し直して初めて要素へばらける
（`apply-whitelist.ps1` の `ConvertFrom-JsonArray` がこれ）。

- ⚠ 併発する罠として、**関数から `return @()` すると「何も出力しない」に化ける**。
  StrictMode 下では受け側が `$null` になり、`.Count` の参照で落ちる。
  **呼び出し側で `@(...)` で受ける**こと。
- PowerShell 7 系では `ConvertFrom-Json` が列挙するので**再現しない**。
  ops スクリプトは 5.1 で動く前提なので、7 で試して大丈夫でも根拠にならない。
