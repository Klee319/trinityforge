...
### ⚠️ 統合版の「空が別次元のまま」はバックエンド側から予防できない（2026-08-24）
統合版クライアントはネザーポータルの霧遷移を始めた状態で Velocity の `/server`（または
リログ）が挟まると、移動先がチャンクだけ読み直して次元変更パケットを送らないため、
現世でもネザーの空を描き続ける（GeyserMC/Geyser#3005 系・上流未修正）。実機再現は
「資源鯖のポータル内に立ったまま /server main」。`/server` はプロキシ組み込みコマンドで
バックエンドからは遮れない＝**予防は構造的に不可能で、着地先での修復しかない**。
Bukkit API に「次元変更パケットの再送」は無いので、修復の唯一の形は
**別環境ワールドへの往復テレポート**（クライアントに次元を再構築させる）。
GeyserExtra の `DimensionHandoffListener`（GeyserAddon `2a5a335`）が実装:
退出時に共有 extension フォルダ `dimension-handoff/<uuid>.json` へ「環境・ポータル内か」を
記録（転送では退出と参加が別バックエンドで起きるため、共有フォルダが唯一の伝達経路）、
参加時に危険な参加だけエンド経由で往復する。手動脱出路は `/fixsky`。
⚠️ 往復ものを実装するときは**往路と復路の間の切断**を必ず設計に入れる —— 戻り先を
先に永続化しないと、切断のタイミング次第でプレイヤーがエンドの足場に置き去りになる。
また復路を速くしすぎない（1秒待つ）—— 高速な二重次元切替は、まさにこの不具合を
起こすパターンそのもの。
