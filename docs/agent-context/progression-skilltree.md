...
3. **8近傍に既存ノードがあるセルは使わない**（コネクタ1セル分の隙間を必ず残す）。
4. **レーンは「行を下から1つずつ閉じ、同じ行ではチェーンの続きを主軸の扇より先に」配る**（2026-08-25 / W-250）。
5. **「1段上へ逃げる」コストは横4レーンぶん**（`DEEPER_ROW_COST`）。行はレベル帯そのものなので、
   帯を飛ばすより横へ遠くへ行く方が読める。

不変条件は `AllSkillTreesProgressionTest`（yml の妥当性）と
`AllSkillTreesLayoutIntegrityTest`（配置の読みやすさ）に全ツリー分の拘束テストとして入っている。

#### 配る順序が見た目を決める（2回直しても再発した所）

**主軸ノードは `layoutAll()` の前に全部トランクへ置かれる。** そのため親が置けた順（根からの深さ順）に
空きレーンを配ると、**どの主軸の扇も、下から伸びてきたチェーンの続きより先にレーンを取る**。
押し出された続きは 2〜4 列も横へずれ、コネクタが斜めに長く走る。**ノード行の間に通路の行は1本しか無い**
...
- **列ずれ**: 親も枝であるノード（＝チェーンの続き）が親と違う列に置かれた数。正しい配置なら 0。
- **融合**: 別々のコネクタが同じ行の隣り合うセルを横向きに通っている箇所の数。正しい配置なら 0。

⚠ **「空きセルの探索順」を直しても順序の問題は直らない**（2026-08-24 の W-218 がこれで、同じ症状が
再報告された）。レーンを配る順序が先に決まるので、探索側からは触れない。

⚠ **座標は yml に無い**（`SkillTreeLayout` が level/parent/role から算出）。
つまり**この領域の修正は jar の再ビルドと再配備だけで実機に届き、config 配備は不要**。
...
- **一覧の lore にもパークの説明（`description`）を出す**（2026-08-06、ユーザー要望）。
  当初は「一覧は"どこにあるか探す"画面だから」と省いていたが、それだと**名前だけでは何をする
  パークか分からず一覧から選べない**（1件ずつ通常モードへ飛んで戻る操作が要る）。整形は
  `NativeSkillTreeMenu.descriptionLines` に集約してあり、**通常モードと一覧が同じメソッドを呼ぶ**。
  同じ文字列を2箇所で別々に整形すると「片方だけ `&` 色コードが素通りする」ような食い違いが生える。
...
| 45-52 | 系統（ルート実績）切替バー。8枠に収まるうちは**左詰め**、9件以上なら選択中を 49（中央）に置いて巡回 |
...
- **系統バーに並ぶのは `AchievementCanvas.branchHeadIds`** = **前提を持たないルートだけ**
  （`parent` 未設定／自分自身／存在しないIDを指しているもの）。**分岐先（ルートの子）は並べない**
  — 2026-08-06 のユーザー確定事項。出荷configの真のルートは `main` 1件だけなので
  **バーはボタン1個になるが、それが仕様どおり**。「バーが1個しかないから壊れている」と判断して
  子を足し戻さないこと（一度その実装で差し戻されている）。系統を増やしたいなら
  `achievements.yml` 側でルートを増やす。
...
## 破壊的な操作の確認は「別画面 ＋ 押した場所を避ける」でないと効かない（2026-08-23、W-188）

プレステージ（そのツリーのパークを全部剥がしてレベルを0に戻す）が「押すだけで走る」と
2回報告された。1回目（2026-08-21）は**同じマスをもう一度クリックで確定**という方式で直したつもりが、
**方式そのものが穴**だった。

- クリック → `pendingPerkId` を立てて **1 tick(50ms)後**に同じ画面を描き直す
- 描き直された画面では**同じスロットが確定ボタンに変わっている**
- **ダブルクリックの2打目は 50ms より後に届く**ので、そのまま確定へ入る
...
2. ⚠ **「はい」を直前に押したスロットへ置かない**（`prestigeYesSlot`）。
   1 だけでは足りない ── 2打目はその新しい画面へ落ちるので、
   たまたま同じスロットなら元に戻る。衝突するときは「はい」と「いいえ」を入れ替えて、
   2打目が必ず**取り消し**になるようにしてある。

### プレステージ枠の警告文は描画時に注入する

生成器が入れる説明文は「永続ボーナス」程度で、レベルが0に戻ることが読み取れない。
`prestigeWarningLore(tree)` を**確認モーダル／ツリー本体／パーク一覧の3画面が共通で**通る。
config（skilltree の yml）には書かない ── 16ツリーぶん書き分けると、必ずどれかで文言が腐る。
