...
## ダンジョンの難易度コンセプト表（2026-08-21 W-181 時点）

**この表は生成物。** 数値を直したら `node tmp/w181-table.js` で作り直して貼り替えること
（出典は `combat/mob-overrides.yml` と `combat/mob-abilities.yml` の出荷値そのもの）。

- **コンセプト** … `物理推奨` = 敵は魔法防御が厚く**物理武器が通る**。攻めは魔法寄りなので**魔法防御が要る**。
  `魔法推奨` はその逆。攻めと受けで要る属性が逆になるのは意図した設計で、
  同じ属性に全部寄せれば済む作りにすると最適解が1本に戻るため。
- **属性差** … 正しい属性を選んだときに与ダメージが何倍になるか（そのダンジョンのモブの平均）。
  2026-08-21 以前は全体の中央値が **1.29 倍**しかなく、396 体中 **43 体は物理と魔法が完全に同値**だった
  （＝属性を選ぶ意味がゼロ）。W-181 で「通る側の耐性 0.05 / 硬い側 0.24〜0.55」へ開いた。
- **HP梯子** … スコープ直下の `max-health-multiplier`。難易度1が最大・難易度10で 1.0 の逓減。
  「ボスの攻撃力を下げたぶんをHPへ振り替える」ための層で、**雑魚にも効く**。
- **ボス通常 / 最悪の技** … Lv100・品質5 の帯最良装備（最大HP 94）が**スキルツリー未投資**で耐えられる発数。
  ツリーの防御ノードを半分取ると 1.5〜3 倍、全開で 2〜6 倍になる（`tmp/w181-verify.js`）。
...
- `damage-percent` の上限を **1.4** に圧縮（`piercing_beam` / `meteor_mark` 2.0 → 1.3 ほか）。
  技のダメージは「そのモブの攻撃力 × この倍率」なので、**1行戻すだけで全ダンジョンが同時に即死級に戻る**。

### 魔法防御は装備だけでは積めない（重装ツリーが正規ルート）

Lv100 帯の最良防具（バニラ系）の **魔法守備は 3.9・魔法耐性 0%**。一方で魔法防御に特化した
**守護/魔織シリーズ（`LEATHER_*#2002xx`）は 60 部位すべてに `max-health` が無い**ので、
そちらへ乗り換えると最大HPが **94 → 20** まで落ちる（＝戦士が着替えて対処する道は無い）。
