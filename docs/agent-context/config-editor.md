...
### ⚠ 「入力欄が空欄」は yml ではなく【エディタのプロセスが古い】ことが多い

ラベルと説明文は静的ファイル `public/js/constants.js` からブラウザが**毎回取り直す**が、
値を返す `GET /api/constants` は**起動時に `require` した `lib/constants.js`** に基づく。
したがって「新しい項目の枠と説明は出るのに中身だけ全部空」という状態は、
**yml でも配線でもなく、走りっぱなしのプロセスが機能追加より古い**ことを意味する。

切り分け（推測より先にこれを見る）:

```bash
curl -s http://127.0.0.1:8000/api/constants | python -c "import sys,json;d=json.load(sys.stdin)['constants']['fields'];print(len(d))"
```

返却キーに当該フィールドが無ければ確定。**対処はエディタの再起動だけ**で、yml は触らない。
実例: W-165（2026-08-20）── プロセスの起動が 2026-08-18 13:58、機能追加コミットが同日 19:05 で、
`dungeon-level-reward.*` が 0 件しか返っていなかった。
