...
**難度の方針（2026-07-30 ユーザー確定）: ソロは「頑張ればぎりぎり行ける」**。
つまり各フェーズの弱点は **「対応ロールが居ると明確に楽」だが「居なくても通せる」** 水準に留める。
弱点を突けないと詰む数値（防御率 0.75 級）は使わない。目安は
**適正ロールで所要時間が半分になるが、不適正でも 2 倍かければ倒せる**こと。

| フェーズ | 性格 | 主に効く数値 | 効くロール（居なくても可） |
|---|---|---|---|
| phase_1 | 物理寄りの壁 | `physical.defense-rate: 0.50`、`magical.defense-rate: 0.15` | **剣闘士**（貫通 0.2 が実質 +40% 火力） |
| phase_2 | 魔法寄りの壁 | `physical.defense-rate: 0.15`、`magical.resistance: 0.45` ＋ 中 `flat-defense` | **魔術師**（割合追加 0.3） |
| phase_3 | 総攻撃 | 増援 6 種が同時湧き、`attack.fixed-damage` は**中**（固定ダメは全貫通なので盛りすぎない） | **守衛**（`damage-reduction` + ヘイト 1.5）。ソロなら増援を捌く時間が伸びるだけ |
| phase_4 | 消耗戦 | HP を最大にし、`attack.percent-bonus-damage` を漸増 | 補助職の恒久バフ（HASTE/SPEED/LUCK）と回復 |

⚠ **`fixed-damage` は全防御貫通**なので、ここを高くするとソロ不可に直結する。
「難しくしたい」ときは `fixed-damage` ではなく **HP と増援の数**を増やすこと（時間はかかるが詰まない）。
...
### 決まったこと（2026-07-30 ユーザー確定）

1. **ソース階梯 9 段の長さは問題にしない。** 上位ジャー／瓶は **config で任意の容量（生成量）を
   指定して作れるようにする**方針が確定したので、段数は運用でいくらでも刻める。
   → §3.2 の案 B（`SourceJarConfig` を yml 全 jar 対応にする）が**確定路線**。案 A は採らない。
2. **マルチブロックのパターン判定は作らない。** 上記 1 の config 駆動ジャーで
   「限られた床面積に何を置くか」というパズルは成立するので、形状判定を足す価値が無い。
   儀式コア＋半径 5 のジャー群という現行の疑似マルチブロックをそのまま「炉」と呼んで前面に出す。
3. **束縛者はソロで「頑張ればぎりぎり行ける」水準**（§4 に反映済み）。
   弱点は「対応ロールが居ると楽」まで。`fixed-damage` で難度を作らない。
4. **ネザー／エンドの攻撃力は実機校正で詰める**ことを受理。§3.7 の数値は計算値のまま出荷し、
   実戦データが出てから調整する。
5. **図鑑の「未発見ヒント」は作らない**（tips 系は読まれない、というユーザー判断）。

### ~~残る未決事項~~ → **3件とも解決（2026-08-01 ユーザー確定）**

1. ~~**スレッド厳選の再抽選儀式は、既存のスレッド在庫の価値を壊す。** 導入時の告知が要る。~~
   → **告知は不要。まだテスト鯖なので在庫価値の破壊を心配しなくてよい**（2026-08-01 ユーザー確定）。
   **設計への影響**: 再抽選儀式は既存在庫との互換や移行措置を一切考えずに入れてよい。
   同様に、**厳選まわりの数値・振れ幅・上限は後方互換を気にせず一度で正しい形に置ける**
   （移行パスの実装コストをプランに積まない）。
2. ~~**図鑑報酬の憲法（縦強化にしない）を維持するか。**~~
   → **維持する**（2026-08-01 ユーザー確定）。
   **設計への影響**: `reward-tiers` に `permanent-buffs` を**入れない**。図鑑報酬は
   称号／パーティクル／コスメ／素材に限る。これにより図鑑は「やらないと不利」にならず、
   憲法③の「横（選択肢）」側に留まる。`ShippedAchievementTreeTest` が
   「`permanent-buffs` を持つノードは `goal_worldbinder` ただ1つ」を固定しているので、
   図鑑側に縦強化を足すとビルドが落ちる＝機械で守られている。
3. ~~**`infinity_source_core` の使い回しを許容するか**（§3.4）。~~
   → **許容する。ただし「工夫があるとなお良し」**（2026-08-01 ユーザー確定）。
   **設計への影響**: 判定用アイテムを消費させない（＝1個作れば以後も達成判定に使える）前提で書く。
   そのうえで、単なる「持っているだけ」に留めない工夫を足す余地がある。
   工夫の候補（プラン側で1つ選ぶ）:
   - **設置して機能させる**: 炉（儀式コア＋半径5のジャー群）の中心に据えると
     周囲のジャー容量／転送レートに補正が乗る。持ち歩くより置いた方が得なので死蔵しない。
   - **段階的に育つ**: 累計ソース量に応じて見た目と称号が変わる（消費はしない）。
   - **貸し出し/共有**: 設置者以外もその炉の恩恵を受けられる。マルチの動機になる。
