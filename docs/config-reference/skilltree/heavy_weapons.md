...
出荷config `TrinityForge/src/main/resources/skilltree/heavy_weapons.yml` の本文コメント(先頭ヘッダを除く)をここに移設したもの。
...
```
  # ---- 主軸 A〜E ----
  # ---- 派生ノード（上段：ノックバック/攻撃範囲） ----
  # ---- ギリシャ文字路線（各主軸の下・alpha=ダメージ / beta=貫通率 / gamma=固定ダメージ・列ごとに排他） ----
```
